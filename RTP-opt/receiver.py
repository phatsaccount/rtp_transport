...
def receiver(receiver_ip, receiver_port, window_size):
    ...
    expected_seq_num = 0
    buffer = {}
    end_seq_num = None  # Track END packet's sequence number
    ...
    while True:
        ...
        if pkt_header.type == 0:  # START
            if not in_connection:
                ...
                end_seq_num = None
                ack_header = PacketHeader(type=3, seq_num=0, length=0)
            ...
            if in_connection:
                ...
                #end ack
                ack_header = PacketHeader(type=3, seq_num=end_seq_num, length=0)
                ack_header.checksum = compute_checksum(ack_header)
                s.sendto(bytes(ack_header), address)
            ...
            if in_connection:
                ...
                # Send ACK for every DATA packet received, with seq_num = DATA seq_num
